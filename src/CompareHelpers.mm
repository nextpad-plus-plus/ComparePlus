...
// Icon RGBA data headers — these must be copied from the Windows
// Icons/ directory into the macOS project include path before building.
// They define static const unsigned char arrays for margin symbols.
#if __has_include("icon_added.h")
  #include "icon_added.h"
  #include "icon_removed.h"
  #include "icon_changed.h"
  #include "icon_moved.h"
  #include "icon_arrows.h"
  #define HAVE_ICON_DATA 1
#else
  #define HAVE_ICON_DATA 0
#endif
...
// These globals are defined in ComparePlus.mm
...
constexpr int cBlinkCount        = 3;
constexpr int cBlinkInterval_ms  = 100;
...
{
    CallScintilla(MAIN_VIEW, SCI_MARKERDEFINERGBAIMAGE, type, (intptr_t)rgba);
    CallScintilla(SUB_VIEW,  SCI_MARKERDEFINERGBAIMAGE, type, (intptr_t)rgba);
}
...
// =====================================================================
//  Visible / bookmark / marker queries
// =====================================================================
...
std::vector<intptr_t> getVisibleLines(int view, bool skipFirstLine)
{
    ...
    const auto sel = getSelectionLines(view);
...
intptr_t otherViewMatchingLine(int view, intptr_t line, intptr_t adjustment, bool check)
{
    ...
    const intptr_t otherLine = getDocLineFromVisible(otherView,
            getVisibleFromDocLine(view, line) + adjustment);

    if (check && (otherLine < otherLineCount) &&
            (otherViewMatchingLine(otherView, otherLine, -adjustment) != line))
        return -1;
...
void activateBufferID(intptr_t buffId)
{
    if (buffId != getCurrentBuffId())
    {
        ...
        nppData._sendMessage(nppData._nppHandle, NPPM_ACTIVATEDOC, index >> 30, index & 0x3FFFFFFF);
    }
}
...
std::pair<intptr_t, intptr_t> getSelectionLines(int view)
{
    if (isSelectionVertical(view) || isMultiSelection(view))
        return std::make_pair((intptr_t)-1, (intptr_t)-1);
...
// =====================================================================
//  Arrow symbols, blinking, centering
// =====================================================================

int showArrowSymbol(int view, intptr_t line, bool down)
{
#if HAVE_ICON_DATA
    const unsigned char* rgba = down ? icon_arrow_down : icon_arrow_up;
    CallScintilla(view, SCI_MARKERDEFINERGBAIMAGE, MARKER_ARROW_SYMBOL, (intptr_t)rgba);
#endif
...
void blinkLine(int view, intptr_t line)
{
    ...
    for (int i = cBlinkCount; ;)
    {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(cBlinkInterval_ms));
...
void blinkRange(int view, intptr_t startPos, intptr_t endPos)
{
    ...
    const auto sel = getSelection(view);
...
{
    const intptr_t firstVisible = getVisibleFromDocLine(view, line) -
            CallScintilla(view, SCI_LINESONSCREEN, 0, 0) / 2;
...
// =====================================================================
//  Indicator (changed-text highlighting)
// =====================================================================
...
void setCompareView(int view, bool showMargin, int blankColor, int caretLineTransp)
{
    if (!compareMode[view])
    {
        ...
        {
            CallScintilla(view, SCI_SETMARGINMASKN,      marginNum,
                          (intptr_t)(MARKER_MASK_SYMBOL | MARKER_MASK_ARROW));
...
bool isDarkMode()
{
    if (nppData._sendMessage(nppData._nppHandle, NPPM_ISDARKMODEENABLED, 0, 0))
        return true;

    const int bg = static_cast<int>(
        nppData._sendMessage(nppData._nppHandle, NPPM_GETEDITORDEFAULTBACKGROUNDCOLOR, 0, 0));
...
{
    const int mainCount = static_cast<int>(
        nppData._sendMessage(nppData._nppHandle, NPPM_GETNBOPENFILES, 0, PRIMARY_VIEW));
    const int subCount = static_cast<int>(
        nppData._sendMessage(nppData._nppHandle, NPPM_GETNBOPENFILES, 0, SECOND_VIEW));

    std::vector<std::string> files;
    files.reserve(mainCount + subCount);

    for (int i = 0; i < mainCount; ++i)
    {
        intptr_t buffId = nppData._sendMessage(nppData._nppHandle, NPPM_GETBUFFERIDFROMPOS, i, MAIN_VIEW);
        char buf[1024] = {};
    ...
    {
        intptr_t buffId = nppData._sendMessage(nppData._nppHandle, NPPM_GETBUFFERIDFROMPOS, j, SUB_VIEW);
        char buf[1024] = {};
        nppData._sendMessage(nppData._nppHandle, NPPM_GETFULLPATHFROMBUFFERID, buffId, (intptr_t)buf);
        files.emplace_back(buf);
    }
...
// =====================================================================
//  setStyles — configure all diff marker colors and icons
// =====================================================================

void setStyles(UserSettings& settings)
{
    // NPPM_GETEDITORDEFAULTBACKGROUNDCOLOR is not available on macOS.
    // Get the editor background from Scintilla's STYLE_DEFAULT instead.
...
// =====================================================================
//  clearWindow — remove all compare markers from a view
// =====================================================================
        ...
        bool excludeNewLine)
{
    ...
    if ((startLine < 0) || (endLine > lastLine) || (startLine > endLine) ||
            !isLineMarked(view, startLine, markMask))
        return std::make_pair((intptr_t)-1, (intptr_t)-1);
...
std::vector<int> getMarkers(int view, intptr_t startLine, intptr_t length, int markMask, bool clearMarkers)
{
    ...
    for (intptr_t line = CallScintilla(view, SCI_MARKERPREVIOUS, startLine + length - 1, markMask);
         line >= startLine;
...
std::vector<char> getText(int view, intptr_t startPos, intptr_t endPos)
{
    ...
    CallScintilla(view, SCI_GETTEXTRANGEFULL, 0, (intptr_t)&tr);
    ...
    intptr_t searchStartLine)
{
    ...
    else
    {
        ...
        CallScintilla(view, SCI_SETSEARCHFLAGS, (uintptr_t)SCFIND_MATCHCASE, 0);
        pos = CallScintilla(view, SCI_SEARCHINTARGET, txtToReplace.size(), (intptr_t)txtToReplace.c_str());
    }

    if (pos >= 0)
    {
        CallScintilla(view, SCI_REPLACETARGETMINIMAL, (uintptr_t)-1, (intptr_t)replacementTxt.c_str());
...
void addBlankSection(int view, intptr_t line, intptr_t length, intptr_t textLinePos, const char* text)
{
    ...
    CallScintilla(view, SCI_ANNOTATIONSETTEXT, getPreviousUnhiddenLine(view, line), (intptr_t)blank.data());
}
...
void addBlankSectionAfter(int view, intptr_t line, intptr_t length)
{
    ...
    CallScintilla(view, SCI_ANNOTATIONSETTEXT, getUnhiddenLine(view, line), (intptr_t)blank.data());
}
