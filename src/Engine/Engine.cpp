 ...
 *
 * Ported to macOS: replaced Windows APIs with portable C++17 equivalents,
 * boost::wregex with std::regex, wchar_t/wstring with char/string (UTF-8).
 * All Scintilla calls go through the extern CallScintilla() function.
 * Helper functions (getLineStart, getText, etc.) are declared extern and
 * must be implemented by the host plugin.
 */
...
// ---------------------------------------------------------------------------
// Scintilla message constants (from Scintilla.h) used directly in this file
// ---------------------------------------------------------------------------
...
#ifndef INDIC_ROUNDBOX
#define INDIC_ROUNDBOX                  7
#endif


// ---------------------------------------------------------------------------
// Marker masks — must match the host plugin's definitions.
// These are the standard ComparePlus marker definitions.
// ---------------------------------------------------------------------------
enum Marker_t
{
...
// ---------------------------------------------------------------------------
// External helper functions — implemented by the host plugin.
// These wrap Scintilla calls and UI operations.
// ---------------------------------------------------------------------------
extern intptr_t getLineStart(int view, intptr_t line);
extern intptr_t getLineEnd(int view, intptr_t line);
extern intptr_t getLinesCount(int view);
extern bool isLineEmpty(int view, intptr_t line);
extern bool isLineHidden(int view, intptr_t line);
extern bool isLineFolded(int view, intptr_t line);
extern bool getNextLineAfterFold(int view, intptr_t* line);
extern intptr_t getUnhiddenLine(int view, intptr_t line);
extern std::vector<char> getText(int view, intptr_t startPos, intptr_t endPos);
extern void markTextAsChanged(int view, intptr_t start, intptr_t length, int color);
extern void clearChangedIndicator(int view, intptr_t start, intptr_t length);
extern void clearWindow(int view, bool clearIndicators = true);


static progress_ptr getProgress()
{
	if (globalProgress)
		return globalProgress;

	// Fallback no-op
	static auto noop = std::make_shared<ProgressIf>();
	return noop;
}


// ---------------------------------------------------------------------------
// Logging stubs — disabled on macOS (no DLOG defined by default)
// ---------------------------------------------------------------------------
#ifndef DLOG
#define LOGD(...)
...
// On macOS with UTF-8, Char wraps a single byte (or could wrap a codepoint).
// For the diff engine's purposes, we compare byte-by-byte just like the original
// compared wchar_t-by-wchar_t. Since Scintilla on Cocoa uses UTF-8 byte positions,
// this is correct and no conversion is needed.
struct Char
{
...
// Portable character classification for UTF-8 bytes
inline charType getCharType(char letter)
{
	if (letter == ' ' || letter == '\t')
		return charType::SPACECHAR;

	unsigned char uc = static_cast<unsigned char>(letter);
	if (std::isalnum(uc) || letter == '_')
...
// Portable in-place lowercase for a byte vector (UTF-8 safe for ASCII range)
inline void toLowerCase(std::vector<char>& text)
{
	for (auto& c : text)
	{
...
// ---------------------------------------------------------------------------
// Hash a section range of a line, respecting ignore options.
// On macOS we work directly with UTF-8 bytes (char), not wchar_t.
// ---------------------------------------------------------------------------
inline uint64_t getSectionRangeHash(uint64_t hashSeed, std::vector<char>& sec, intptr_t pos, intptr_t endPos,
		const CompareOptions& options)
{
	...
	{
		for (intptr_t i = pos; i < endPos; ++i)
		{
			unsigned char uc = static_cast<unsigned char>(sec[i]);
			if (uc < 128)
				sec[i] = static_cast<char>(std::tolower(uc));
		}
	}

	for (; pos < endPos; ++pos)
	{
		if (options.ignoreAllSpaces && (sec[pos] == ' ' || sec[pos] == '\t'))
			continue;

		if (options.ignoreChangedSpaces && (sec[pos] == ' ' || sec[pos] == '\t'))
		{
			hashSeed = Hash(hashSeed, ' ');

			while (++pos < endPos && (sec[pos] == ' ' || sec[pos] == '\t'));
...
// ---------------------------------------------------------------------------
// Regex-based line hashing — uses std::regex on UTF-8 strings directly.
// No wchar_t conversion needed on macOS.
// ---------------------------------------------------------------------------
uint64_t getRegexIgnoreLineHash(int view, intptr_t off, uint64_t hashSeed, const std::vector<char>& line,
	const CompareOptions& options)
{
	...
	// Work with a mutable copy for case folding
	std::string lineStr(line.begin(), line.end());

	std::sregex_iterator		rit(lineStr.begin(), lineStr.end(), *options.ignoreRegex);
	const std::sregex_iterator	rend;

	if (options.invertRegex && (rit != rend || !options.inclRegexNomatchLines))
	{
		std::vector<char> sec(lineStr.begin(), lineStr.end());

		for (; rit != rend; ++rit)
		{
			intptr_t matchPos = static_cast<intptr_t>(rit->position());
			intptr_t matchLen = static_cast<intptr_t>(rit->length());
			hashSeed = getSectionRangeHash(hashSeed, sec, matchPos, matchPos + matchLen, options);

			if (options.highlightRegexIgnores)
			{
				// Mark the non-matched parts as blank
				// (simplified: just mark matched portions for now)
			}
		}

		if (options.highlightRegexIgnores)
		{
			// Mark remaining as blank (simplified)
		}
	}
	else
	{
		std::vector<char> sec(lineStr.begin(), lineStr.end());
		intptr_t pos = 0;
		intptr_t endPos = static_cast<intptr_t>(lineStr.size()) - 1;

		if (options.ignoreChangedSpaces && pos < endPos && (sec[pos] == ' ' || sec[pos] == '\t'))
		{
			while (++pos < endPos && (sec[pos] == ' ' || sec[pos] == '\t'));
		...
		{
			intptr_t matchPos = static_cast<intptr_t>(rit->position());
			hashSeed = getSectionRangeHash(hashSeed, sec, pos, matchPos, options);

			pos = matchPos + static_cast<intptr_t>(rit->length());
			++rit;
		}
		...
		if (options.ignoreChangedSpaces)
		{
			...
			while (--eolPos >= pos && (sec[eolPos] == '\n' || sec[eolPos] == '\r'));

			if (++eolPos > pos)
			{
				intptr_t p = eolPos;
				while (--p >= pos && (sec[p] == ' ' || sec[p] == '\t'));

				if (++p > pos)
					hashSeed = getSectionRangeHash(hashSeed, sec, pos, p, options);
			}

			hashSeed = getSectionRangeHash(hashSeed, sec, eolPos, endPos, options);
		}
		else
		{
			hashSeed = getSectionRangeHash(hashSeed, sec, pos, endPos, options);
		}
	}
...
// ---------------------------------------------------------------------------
// getLines — reads all lines from a Scintilla view into the DocCmpInfo.
// ---------------------------------------------------------------------------
void getLines(DocCmpInfo& doc, const CompareOptions& options)
{
	constexpr int monitorCancelEveryXLine = 1000;

	progress_ptr progress = getProgress();
	...
	for (intptr_t secLine = 0; secLine < doc.section.len; ++secLine)
	{
		...
		if (lineStart < lineEnd)
		{
			...
			{
				newLine.hash = getRegexIgnoreLineHash(doc.view, lineStart, newLine.hash, line, options);
...
// ---------------------------------------------------------------------------
// Word and character extraction — on macOS, everything is UTF-8 bytes.
// No wchar_t conversion needed.
// ---------------------------------------------------------------------------

inline void getSectionRangeWords(std::vector<Word>& words, std::vector<char>& line, intptr_t pos, intptr_t endPos,
		const CompareOptions& options)
{
	if (pos >= endPos)
		return;
	...
	if (options.ignoreCase)
	{
		for (intptr_t i = pos; i < endPos; ++i)
		{
			...
			if (uc < 128)
				line[i] = static_cast<char>(std::tolower(uc));
		}
	}

	charType currentWordType = getCharType(line[pos]);

	Word word {pos, 1};

	if (options.ignoreChangedSpaces && currentWordType == charType::SPACECHAR)
		word.hash = Hash(cHashSeed, ' ');
	else
		word.hash = Hash(cHashSeed, line[pos]);

	for (; ++pos < endPos;)
	{
		const charType newWordType = getCharType(line[pos]);
...
std::vector<Word> getRegexIgnoreLineWords(std::vector<char>& line, const CompareOptions& options)
{
	...
	else
	{
		...
		if (options.ignoreChangedSpaces && (line[endPos] == ' ' || line[endPos] == '\t'))
		{
			while (--endPos >= pos && (line[endPos] == ' ' || line[endPos] == '\t'));
...
std::vector<Word> getLineWords(int view, intptr_t docLine, const CompareOptions& options)
{
	...
	if (options.ignoreRegex)
	{
		words = getRegexIgnoreLineWords(line, options);
	}
	else
	{
		intptr_t pos = 0;
		intptr_t endPos = static_cast<intptr_t>(line.size());
		...
		getSectionRangeWords(words, line, pos, endPos, options);
	}

	// On macOS with UTF-8, Scintilla byte positions match directly — no recalculation needed.
...
inline void getSectionRangeChars(std::vector<Char>& chars, std::vector<char>& sec, intptr_t pos, intptr_t endPos,
		const CompareOptions& options)
{
	if (pos >= endPos)
		return;
	...
	for (; pos < endPos; ++pos)
	{
		const charType typeOfChar = getCharType(sec[pos]);
		...
		{
			chars.emplace_back(' ', pos);

			while (++pos < endPos && getCharType(sec[pos]) == charType::SPACECHAR);
...
std::vector<Char> getSectionChars(int view, intptr_t secStart, intptr_t secEnd, const CompareOptions& options)
{
	...
	getSectionRangeChars(chars, sec, 0, len, options);
	...
	const CompareOptions& options)
{
	...
	{
		for (; rit != rend; ++rit)
			getSectionRangeChars(chars, line, rit->position(), rit->position() + rit->length(), options);
	}
	else
	{
		intptr_t pos = 0;
		...
		while (rit != rend)
		{
			getSectionRangeChars(chars, line, pos, rit->position(), options);
		...
		if (endPos >= pos)
			getSectionRangeChars(chars, line, pos, endPos + 1, options);
	}
		...
		if (lineStart < lineEnd)
		{
			...
			else
			{
				...
				if (options.ignoreChangedSpaces && !chars[blockLine].empty())
				{
					...
					for (; itr != chars[blockLine].end() && (itr->ch == ' ' || itr->ch == '\t'); ++itr);

					if (itr != chars[blockLine].begin())
						chars[blockLine].erase(chars[blockLine].begin(), itr);

					if (!chars[blockLine].empty())
					{
						size_t i = chars[blockLine].size() - 1;

						for (; i > 0 && (chars[blockLine][i] == ' ' || chars[blockLine][i] == '\t'); --i);

						if (i + 1 != chars[blockLine].size())
							chars[blockLine].erase(chars[blockLine].begin() + static_cast<intptr_t>(i) + 1, chars[blockLine].end());
					}
				}
			}
		}
...
inline intptr_t matchBeginEnd(diffInfo& blockDiff1, diffInfo& blockDiff2,
		...
		std::function<bool(const char)>&& charFilter_fn)
{
	const intptr_t minSecSize = std::min(static_cast<intptr_t>(sec1.size()), static_cast<intptr_t>(sec2.size()));
...
// ---------------------------------------------------------------------------
// Line-level comparison within changed blocks
// ---------------------------------------------------------------------------
		...
		const std::map<intptr_t, intptr_t>& lineMappings, const CompareOptions& options)
{
	for (const auto& lm: lineMappings)
	{
			...
			else
			{
				...
				if ((i + 1 < lineDiffsSize) && (lineDiffs[i + 1].type == diff_type::DIFF_IN_2))
				{
					...
					else
					{
						const intptr_t matches =
								matchBeginEnd(blockDiff1, blockDiff2, sec1, sec2, off1, off2, end1, end2,
									[](const char ch) { return (getCharType(ch) != charType::ALPHANUMCHAR); });
		...
		if (lineLen1 > 0 && lineLen2 > 0 &&
			((totalLineMatchLen * 100) / std::max(lineLen1, lineLen2)) < options.changedResemblPercent)
		{
...
// ---------------------------------------------------------------------------
// Convergence calculation for matching changed lines
// ---------------------------------------------------------------------------
		...
		const diffInfo& blockDiff1, const diffInfo& blockDiff2, const CompareOptions& options)
{
	...
	for (intptr_t line1 = 0; line1 < linesCount1; ++line1)
	{
		...
		for (intptr_t line2 = 0; line2 < linesCount2; ++line2)
		{
			...
			const intptr_t minSize = std::min(static_cast<intptr_t>(chunk1[line1].size()),
											  static_cast<intptr_t>(chunk2[line2].size()));
			const intptr_t maxSize = std::max(static_cast<intptr_t>(chunk1[line1].size()),
											  static_cast<intptr_t>(chunk2[line2].size()));
			...
			if (!options.detectCharDiffs || !options.ignoreAllSpaces)
			{
				...
				const auto wordDiffs = DiffCalc<Word>(words1, words2[line2],
						std::bind(&ProgressIf::IsCancelled, progress))();
...
void findSubBlockDiffs(CompareInfo& cmpInfo, const CompareOptions& options)
{
	...
	const int threadsCount = static_cast<int>(std::thread::hardware_concurrency()) - 2;
...
void markLineDiffs(const CompareInfo& cmpInfo, const diffInfo& bd, intptr_t lineIdx)
{
	...
	int color = (cmpInfo.doc1.blockDiffMask == MARKER_MASK_ADDED) ?
			compareColors.added_part : compareColors.removed_part;
	...
	for (const auto& change: bd.info.changedLines[lineIdx].changes)
		markTextAsChanged(cmpInfo.doc1.view, linePos + change.off, change.len,
						change.moved ? compareColors.moved_part : color);
...
// ---------------------------------------------------------------------------
// Mark all diffs and compute alignment info
// ---------------------------------------------------------------------------
					...
					if (cmpInfo.doc1.section.len || cmpInfo.doc2.section.len)
					{
						...
						if (options.neverMarkIgnored && cmpInfo.doc1.section.len && cmpInfo.doc2.section.len)
						{
							...
							if (!alignLines1.empty())
							{
								...
								maxLines = std::min(static_cast<intptr_t>(alignLines1.size()),
													static_cast<intptr_t>(alignLines2.size()));
...
CompareResult runFindUnique(const CompareOptions& options, CompareSummary& summary)
{
	...
	for (const auto& uniqueLine: doc2UniqueLines)
	{
		...
		if (doc2.blockDiffMask == MARKER_MASK_ADDED)
			summary.added += static_cast<intptr_t>(uniqueLine.second.size());
		else
			summary.removed += static_cast<intptr_t>(uniqueLine.second.size());
	}
...
CompareResult compareViews(const CompareOptions& options, const char* progressInfo, CompareSummary& summary)
{
	...
	// On macOS, progress is managed by the host plugin (no Windows dialog).
	// We just ensure the progress object exists.
	(void)progressInfo;

	try
	{
		// Clear any previous change indicators
		clearChangedIndicator(MAIN_VIEW, 0, CallScintilla(MAIN_VIEW, SCI_GETLENGTH, 0, 0));
		clearChangedIndicator(SUB_VIEW, 0, CallScintilla(SUB_VIEW, SCI_GETLENGTH, 0, 0));
	...
	catch (std::exception& e)
	{
		...
		// On macOS, error reporting is handled by the host plugin
		(void)e;
	}
	catch (...)
	{
